...
#if _MAX_SS != _MIN_SS						
	DWORD ssize;
	DRESULT result = disk_ioctl(fs->drv, GET_SECTOR_SIZE, &ssize);
	if ( (result != RES_OK) || (ssize < _MIN_SS) || (ssize > _MAX_SS) )
		return FR_DISK_ERR;
	else
		SS(fs) = (WORD)ssize;
#endif
...
)
{
...
#if _MAX_SS != _MIN_SS						
	DWORD ssize;
	DRESULT result = disk_ioctl(pdrv, GET_SECTOR_SIZE, &ssize);
	if ( (result != RES_OK) || (ssize < _MIN_SS) || (ssize > _MAX_SS) )
		return FR_DISK_ERR;
	else
		SS(fs) = (WORD)ssize;
	...
	if (_MULTI_PARTITION && part) {
		...
		result = disk_ioctl(pdrv, GET_SECTOR_COUNT, &n_vol);
		if (result != RES_OK || n_vol < 128)
			return FR_DISK_ERR;
